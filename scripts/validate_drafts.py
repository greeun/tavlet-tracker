...
SECRET_PATTERNS = [
    ...
    # 홈 절대경로 3종에는 선행 문맥 조건을 건다. 경로가 실제로 그 자리에서 시작할
    # 때에만 잡기 위해서다. 조건이 없으면 레포 상대 경로 한가운데의
    # `src/app/[locale]/home/v1/page.tsx` 가 `/home/v1/` 로 오탐된다(2026-09-04 실측).
    # (?<![^\s"\'`(\[]) 는 "앞 문자가 공백·따옴표·백틱·여는 괄호가 아니면 실패"이며
    # 문자열 시작에서는 볼 문자가 없어 성공한다. 소비하지 않으므로 스니펫도 깨끗하다.
    (r"(?<![^\s\"'`(\[])/Users/[A-Za-z0-9._\-]+/", "사용자 홈 절대경로 (macOS)"),
    (r"(?<![^\s\"'`(\[])/home/[A-Za-z0-9._\-]+/", "사용자 홈 절대경로 (Linux)"),
    (r"(?<![^\s\"'`(\[])[A-Za-z]:\\\\Users\\\\[A-Za-z0-9._\-]+\\\\", "사용자 홈 절대경로 (Windows)"),
]
