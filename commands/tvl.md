...
4. **tavlet 레포 판정.** 현재 세션이 tavlet 레포(레포 루트에 `agent/mcp/board/server.ts` 와 `agent/skills/tavlet-board-submit/SKILL.md` 가 **둘 다** 존재)이면, 레포 내부 파이프라인(`/tav` → S0 등록 → S1 인테이크 → S3 처리 → S4 결과등록)으로 **핸드오프를 먼저 제안**하고, 사용자가 "그래도 tavlet-tracker로 진행"이라고 확정하기 전에는 진행하지 않는다.
